...
import { CurrencyAmount, FINDORA, SwapParameters, Token, Trade, TradeOptions, TradeType } from 'tropic-swap'
...
export default function v1SwapArguments(trade: Trade, options: Omit<TradeOptions, 'feeOnTransfer'>): SwapParameters {
  ...
  const inputETH = trade.inputAmount.currency === FINDORA
  const outputETH = trade.outputAmount.currency === FINDORA
  if (inputETH && outputETH) throw new Error('FINDORA to FINDORA')
